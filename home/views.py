...
def signup(req):
    if req.method=='POST':
        ...
        hname_trial=req.POST.get('hname')
        if hname_trial[0].isdigit():
            return render(req,'home/signup.html',{'error':'Hospital name cannot start with a numeric value'})
        else:
            user1.hname=hname_trial    
        user1.username=req.POST.get('username')
        if len(req.POST.get('password'))>=8 and len(req.POST.get('password'))<=15:
            user1.password=hash_password(req.POST.get('password'))
        else:
            return render(req,'home/signup.html',{'error':'Length of password must be minimum 8 characters and maximum 15 characters'})
...
def blood1(req):
    ...
    if current_user['status']==True:
        if req.method=='POST':
        ...
        elif current_user['hname']==req.GET['name'] and current_user['stock']==0:
            ...
            return render(req,'home/blood.html',{'error':'The required entity is present with you!'})
    else:
        return redirect('home:login')
...
def stocks(req):
    if current_user['status']==True:
        od=blood.objects.raw("select * from home_blood where hname=%s",[current_user['hname']])
        om=medicine.objects.raw("select * from home_medicine where hname=%s",[current_user['hname']])
        return render(req,"home/stocks.html",{'od':od,'om':om})
    else:
        return redirect('home:four')
